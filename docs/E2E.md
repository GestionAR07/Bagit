...
El guard bloquea el dominio histórico `pedilo.store`, la producción actual `bagitar.netlify.app`, `bagit.ar`, `bagit.com.ar` y todos sus subdominios. Una excepción READ_ONLY hacia un host DEV remoto requiere opt-in explícito y **nunca** habilita WRITE_DEV ni permite esos hosts productivos.
...
La baseline de logging operativo seguro y Sentry remoto ya están implementados. Supabase PROD y Netlify productivo también están disponibles. Siguen pendientes backup/restore y QA en dispositivos reales. Ver [`OPERATIONS.md`](./OPERATIONS.md).
