# Google OAuth branding and identity (Bag It)

This file is the **canonical checklist** for Google login branding and for
reusing an existing Bag It account. Credentials never live in this repository.

Public app name in product copy: **Bag It** (`src/lib/app-info.ts`).
...
Bag It can override from Next.js.
...
2. Set **App name** to `Bag It` (exactly that public brand, not the GitHub
...
button in Bag It already says **Continuar con Google**; it does not print the
...
Bag It **does not** copy `merchant_users` or platform roles onto that new UUID.
...
1. Attach a domain you own (for example `auth.bagit.ar`) as the Supabase
   **custom Auth domain**.
2. Update the Google authorized redirect URI to
   `https://auth.bagit.ar/auth/v1/callback`.
...
## Identity reuse implemented in Bag It
...
2. If the Auth Admin API is available (`SUPABASE_SECRET_KEY` on the server),
   Bag It looks for **another** `auth.users` row with the same email. A hit is
...
4. If the Auth trigger raced (`PROFILE_MISSING`), Bag It creates the missing
